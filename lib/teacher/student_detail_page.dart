...
class StudentDetailPage extends StatelessWidget {
  ...
  Widget _buildPerformanceCard(
    ...
    final double wordReadingScore =
        ((passageWordCount - miscues) / passageWordCount) * 100;
    final double comprehensionScore =
        (logData['totalScore'] != null && logData['totalQuestions'] != null)
    ...
    final oralReadingProfile = _determineOralReadingProfile(
      wordReadingScore,
      comprehensionScore,
    );
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 10),
                    Text(
                      'Word Reading Score: ${wordReadingScore.toStringAsFixed(2)}% ($wordReadingLevel)',
                    ),
                    Text(
                      'Comprehension Score: ${comprehensionScore.toStringAsFixed(2)}% ($comprehensionLevel)',
                    ),
                    Text(
                      'Reading Rate: ${readingSpeed.toStringAsFixed(2)} words per minute',
                    ),
                    Text(
                      'Reading Time: ${logData['readingTime'] != null ? _formatDuration(logData['readingTime']) : 'N/A'}',
                    ),
                        ...
                        if (!showMore) {
                          ...
                          if (miscueDoc.docs.isNotEmpty) {
                            ...
                            if (data.containsKey('miscues')) {
                              final miscuesMap = Map<String, dynamic>.from(
                                data['miscues'],
                              );
                              setState(() {
                                detailedMiscues = miscuesMap.map(
  ...
  String _determineOralReadingProfile(
