  ...
  Future<void> _determineMiscueType() async {
    try {
      ...
      if (!performanceDoc.exists) {
        debugPrint(
          "❌ Error: No StudentPerformance record found for ID ${widget.performanceId}",
        );
        return;
      }
      ...
      String latestTestType =
          performanceData['type']?.toString().toLowerCase() ?? "pretest";
  ...
  Future<void> _saveMiscueScore() async {
    ...
    try {
      ...
      if (querySnapshot.docs.isEmpty) {
        ...
        debugPrint(
          '⚠️ No performance record found for studentId: ${widget.studentId}, type: $selectedType',
        );
      ...
      double wordReadingScore =
          ((passageWordCount - totalMiscueScore) / passageWordCount) * 100;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    ...miscues.keys
                        .map((miscueType) => _buildMiscueRow(miscueType))
                        .toList(),
                  ],
                ),
              ),
      ...
      floatingActionButton: ElevatedButton.icon(
        onPressed: _isSaving
            ? null
            : _saveMiscueScore, // ✅ Disable button while saving
        icon: _isSaving
