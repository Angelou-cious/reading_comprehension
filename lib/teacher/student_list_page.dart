    ...
    return ElevatedButton(
      onPressed: isComplete
          ...
          builder: (context) => const Center(
            child: CircularProgressIndicator(color: Color(0xFF15A323)),
          ),
        );

        try {
          Map<String, String?> ids = await _fetchQuizAndStoryIds(student.id);
          String? quizId = ids['quizId'];
          String? storyId = ids['storyId'];
          String? type = ids['type'];
          String? performanceId = ids['performanceId'];

          Navigator.of(context).pop(); // ✅ Close loading

          if (quizId == null || storyId == null || type == null || performanceId == null) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text("⚠️ Cannot mark miscues. Student hasn't started reading yet."),
              ),
            );
            return;
          }
      ...
      style: ElevatedButton.styleFrom(
        backgroundColor: isComplete ? Colors.grey : const Color(0xFF15A323),
        shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(15.0),
        ),
