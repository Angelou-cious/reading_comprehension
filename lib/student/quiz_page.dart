...
import 'package:phil/widgets/background_reading.dart';
import 'package:phil/student/student_home_page.dart';
import 'package:phil/utils/school_year_util.dart';
...
class _QuizPageState extends State<QuizPage> {
  ...
  // NEW: Track student answers (as choices: 'A', 'B', etc)
  ...
  Future<void> _checkEligibilityAndLoadQuiz() async {
    try {
      ...
      if (quizType.toLowerCase() == 'pretest' && pretestCompleted) {
        throw Exception(
          'Pretest already completed. You are not eligible to retake it.',
        );
      }

      if (quizType.toLowerCase() == 'posttest' &&
          (!pretestCompleted || !posttestAssigned)) {
  ...
  void submitAnswer() async {
    ...
    // NEW: Save the picked answer before moving on!
  ...
  Future<void> showResult() async {
    ...
    double readingSpeed =
        widget.passageWordCount /
        (readingTimeInMinutes > 0 ? readingTimeInMinutes : 1);
    ...
    double wordReadingScore =
        ((widget.passageWordCount - totalMiscues) / widget.passageWordCount) *
        100;
    String comprehensionLevel = determineComprehensionLevel(
      comprehensionScorePercentage,
    );
    String oralReadingProfile = determineOralReadingProfile(
      wordReadingScore,
      comprehensionLevel,
    );
          ...
          TextButton(
            onPressed: () {
              Navigator.of(context).pushAndRemoveUntil(
                MaterialPageRoute(
                  builder: (context) =>
                      StudentHomePage(studentId: widget.studentId),
                ),
                (route) => false,
              );
            },
  ...
  Future<void> _storeQuizResults(
    ...
    try {
      ...
      double wordReadingScore =
          ((passageWordCount - miscues) / passageWordCount) * 100;
      ...
      final performanceRef = FirebaseFirestore.instance.collection(
        'StudentPerformance',
      );
      ...
      if (querySnapshot.docs.isEmpty) {
        await performanceRef.add({
          ...
          'answers': studentAnswers, // NEW: save answers!
      ...
      if (quizType.toLowerCase() == "pretest") {
        await FirebaseFirestore.instance
            .collection('Students')
            .doc(widget.studentId)
            .update({'pretestCompleted': true, 'posttestAssigned': true});
      }
    } catch (e) {
  ...
  String determineOralReadingProfile(
    double wordReadingScore,
    String comprehensionLevel,
  ) {
    if (wordReadingScore >= 97 && comprehensionLevel == "Independent")
      return "Independent";
    if (wordReadingScore >= 90 &&
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Column(
                      children: (question['answers'] as Map<String, dynamic>)
                          .entries
                          .map((entry) {
