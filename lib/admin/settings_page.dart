  ...
  Future<void> _confirmDeleteYear(String year) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete "$year"? This action cannot be undone.'),
        actions: [
          TextButton(
            child: const Text('Cancel'),
    ...
    if (confirmed == true) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('School year $year deleted')),
      );
    }
  }
                  ...
                  children: [
                    Wrap(
                      ...
                      children: _availableYears.map((year) {
                        return Chip(
                          label: Text(year),
                          backgroundColor: year == _activeSchoolYear ? Colors.green[100] : Colors.grey[200],
                          avatar: year == _activeSchoolYear
                              ? const Icon(Icons.check, color: Colors.green)
                              : null,
                          deleteIcon: year != _activeSchoolYear ? const Icon(Icons.close) : null,
                          onDeleted: year != _activeSchoolYear ? () => _confirmDeleteYear(year) : null,
                        );
                      }).toList(),
                    ),
                    const SizedBox(height: 15),
