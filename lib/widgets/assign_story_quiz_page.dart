...
class _AssignStoryQuizPageState extends State<AssignStoryQuizPage> {
  ...
  Future<void> _preloadPretestData() async {
    ...
    List<String> allStudentIds = studentsSnapshot.docs
        .map((doc) => doc.id)
        .toList();
    ...
    for (int i = 0; i < allStudentIds.length; i += 10) {
      ...
      for (int j = 0; j < assignedSnapshots.length; j++) {
        if (assignedSnapshots[j].docs.isNotEmpty) {
          ...
          assignedPretestStoryIds.add(
            assignedSnapshots[j].docs.first['storyId'],
          );
        }
      }
    }
      ...
      future: selectedStoryType == 'Pretest'
          ? Future.wait([
              FirebaseFirestore.instance
                  ...
                  .where(
                    'set',
                    isEqualTo: 'Set ${selectedPassageSet!.toUpperCase()}',
                  )
                  .get(),
    ...
    return DropdownButtonFormField<String>(
      ...
      onChanged: (value) {
        setState(() {
          ...
          selectedStoryQuiz = stories.firstWhere(
            (story) => story['storyId'] == value,
          );
        });
      },
      ...
      return FutureBuilder<List<String>>(
        ...
        builder: (context, snapshot) {
          ...
          if (eligibleIds.isEmpty) {
            return const Center(
              child: Text('All students already have a Pretest assigned.'),
            );
          }
  ...
  void validateBeforeAssigning() async {
    ...
    try {
      ...
      if (quizQuerySnapshot.docs.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('No quiz found for the selected story.'),
          ),
        );
        return;
      }
  ...
  void assignQuizToStudents() async {
    if (selectedStoryQuiz == null || selectedStudents.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select a story, quiz, and students.'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Assignment successful!')));
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Error assigning quiz. Please try again.'),
        ),
      );
    } finally {
      setState(() {
